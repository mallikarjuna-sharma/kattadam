...
import {
  ...
  listLocalEnquiries,
  updateLocalEnquiry,
} from "./internal/local-enquiry-store";
...
export async function catalogCreateEnquiry(row: {
  ...
  try {
    return await appendLocalEnquiry(row);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error("[@kattadam/data-layer] local enquiry store failed:", msg);
    return null;
  }
}
...
export async function adminListEnquiries(filters?: { status?: EnquiryStatus }): Promise<EnquiryRecord[] | null> {
  const b = getServerBackend();
  let remote: EnquiryRecord[] = [];
  if (b) {
    try {
      remote = await b.listEnquiries(filters);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      console.error("[@kattadam/data-layer] listEnquiries failed:", msg);
      if (!useLocalEnquiryFallback()) return null;
    }
  } else if (!useLocalEnquiryFallback()) {
    return null;
  ...
  if (!useLocalEnquiryFallback()) return remote;

  const local = await listLocalEnquiries(filters);
...
export async function adminUpdateEnquiry(
  ...
  if (b) {
    try {
      const updated = await b.updateEnquiry(id, patch);
      if (updated) return updated;
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      console.error("[@kattadam/data-layer] updateEnquiry failed:", msg);
      if (!useLocalEnquiryFallback()) return null;
    }
  } else if (!useLocalEnquiryFallback()) {
    return null;
  ...
  if (!useLocalEnquiryFallback()) return null;
  return updateLocalEnquiry(id, patch);
}
